    ...
    public class IsAsleepMonitor
    {
        ...
        public IsAsleepMonitor(IHaContext ha) {
            ...
            }
            {
                ...
                _0Gbl._myEntities.Sensor.EnvyLastactive.StateChanges().WhenStateIsFor(x => x?.State == "unavailable", TimeSpan.FromMinutes(0), _0Gbl._myScheduler).Subscribe(_ => { SetConditiotnState(condition, false); });
                _0Gbl._myEntities.Sensor.EnvyLastactive.StateChanges().WhenStateIsFor(x => x?.State != "unavailable", TimeSpan.FromMinutes(0), _0Gbl._myScheduler).Subscribe(_ => { SetConditiotnState(condition, true); });
                isAwakeConditions.Add(condition);
            }
            {
