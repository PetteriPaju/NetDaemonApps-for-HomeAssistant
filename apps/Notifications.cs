...
namespace NetDaemonApps.apps
{
    ...
    public class Notifications
    {
        ...
        public Notifications() {
            ...
            A0Gbl._myScheduler.ScheduleCron(gamingtime, () => TTS.Speak("Gaming Time",TTS.TTSPriority.DoNotPlayInGuestMode,A0Gbl._myEntities.InputBoolean.NotificationGamingTime));
            ...
            A0Gbl._myEntities.Sensor.PcMemoryusage.StateChanges().Where(x => x?.New?.State > 90 && x?.Old?.State < 90).Subscribe(_ => { 
                if(lastMemoryAlert < DateTime.Now - TimeSpan.FromMinutes(2))
                {
                    TTS.Speak("Memory Alert, Memory Alert",TTS.TTSPriority.DoNotPlayInGuestMode, A0Gbl._myEntities.InputBoolean.NotificationMemoryAlert);
                    lastMemoryAlert = DateTime.Now;
                }

            });
            A0Gbl._myEntities.Sensor.MotoG8PowerLiteBatteryLevel.StateChanges().Where(x => x?.New?.State < 15 && A0Gbl._myEntities.InputBoolean.Ishome.State == "on" && A0Gbl._myEntities.BinarySensor.MotoG8PowerLiteIsCharging.State == "off").Subscribe(_ => { TTS.Speak("Phone Battery Low", TTS.TTSPriority.DoNotPlayInGuestMode, A0Gbl._myEntities.InputBoolean.NotificationPhoneBattery); });
            A0Gbl._myEntities.Sensor.MotoG8PowerLiteBatteryLevel.StateChanges().Where(x => x?.New?.State < 50 && x?.Old?.State >= 50 && A0Gbl._myEntities.BinarySensor.MotoG8PowerLiteIsCharging.State == "off").Subscribe(_ => { TTS.Speak("Phone Battery Under 50%", TTS.TTSPriority.DoNotPlayInGuestMode, A0Gbl._myEntities.InputBoolean.NotificationPhoneBattery); });


            A0Gbl._myEntities.InputBoolean.Ishome.StateChanges().Where(x => x.New.IsOn()).Subscribe(_ => {
