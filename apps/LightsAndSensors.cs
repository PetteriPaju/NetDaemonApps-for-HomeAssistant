...
namespace NetDaemonApps.apps
{
    ...
    public class LightsAndSensors
    {
        ...
        public LightsAndSensors(IHaContext ha)
        {
            ...
            SubcribeLightOff(_myEntities.BinarySensor.HallwaySensorOccupancy, _myEntities.Light.HallwayLight, defaulMotionTimeout, null);
        ...
        private void SubcribeLightOn(BinarySensorEntity sensor, LightEntity light)
        {
            sensor.StateChanges().Where(e => e.New?.State == "on").Subscribe(_ => { light.TurnOn(); });
        }

        private void SubcribeLightOff(BinarySensorEntity sensor, LightEntity light, TimeSpan offTime, Func<bool>? extraConditions)
        {
